...
### 核心基础设施与安全 (Core Infrastructure & Security)

*   **用户认证体系 (User Authentication):** 引入完整的用户认证体系（如用户名/密码或OAuth），使用持久化用户ID替代临时随机ID。这是实现跨设备同步、持久好友列表和离线消息的基础。
*   **离线消息 (Offline Messaging):** 实现离线消息存储与转发机制，确保用户在不在线时也能接收消息，提升通信的可靠性。
*   **端到端加密 (E2EE):** 为 DataChannel 消息实现额外的端到端加密 (E2EE) 层，确保只有通信双方可以解密消息内容，提升隐私和安全性。

### 核心聊天体验增强 (Core Chat Experience Enhancements)

*   **丰富的消息交互 (Rich Message Interaction):**
    *   **消息状态：** 实现完整的已发送/已送达/已读回执链条。
    *   **消息编辑与引用：** 增加消息编辑、引用回复功能，提供更丰富的聊天交互方式。
*   **文件传输优化 (File Transfer Optimization):** 为文件传输增加实时进度条和状态显示（上传中、发送中、已完成），并提供取消传输的选项，优化大文件共享体验。

### 群组与高级通信 (Group & Advanced Communication)

*   **去中心化群聊：** 探索用于较小群组的完全网状 WebRTC 连接，或用于较大群组的 GossipSub 类协议。
*   **群组音视频通话：** 集成 SFU（选择性转发单元），以支持更高效、更大规模的多人音视频通话。

### 高级AI功能 (Advanced AI Features)

*   **AI Agent 与高级记忆：**
    *   **自主记忆管理：** 在记忆书功能基础上，让 AI 能够自主决定何时及记录何事，创造更自然的长期记忆。
    *   **工具调用与任务执行 (Tool Use & Task Execution):** 大幅扩展 MCP 工具集，实现工具链（连续调用多个工具解决复杂问题）和自主规划能力，使 AI 成为能够执行具体任务的智能代理。
