...
const isMemoryBookEnabled = (setId) => {
  return !!memoryStore.elementSets.find(s => s.id === setId)?.books?.[props.contactId]?.enabled;
};

const toggleMemoryBook = (setId, isEnabled) => {
  memoryStore.setMemoryBookEnabled(setId, props.contactId, isEnabled);
};

const generateMemory = (setId) => {
  memoryStore.generateMemoryBook(setId, props.contactId);
};

const saveMemory = (setId, content) => {
  memoryStore.saveMemoryBookContent(setId, props.contactId, content);
};
