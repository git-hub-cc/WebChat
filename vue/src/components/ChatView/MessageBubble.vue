<template>
  <div class="message-wrapper" :class="wrapperClasses" @contextmenu.prevent="showContextMenu" @dblclick="showContextMenu">
    <!-- MODIFICATION: Handle call-log subType for system messages -->
...
// --- NEW: Computed property for call icon ---
const callIcon = computed(() => {
  if (props.message.subType !== 'call-log') return '';
  const { callType, callerId } = props.message.callData;
  const wasOutgoing = callerId === userStore.userId;
  if (callType === 'audio') return wasOutgoing ? '📞' : '📞'; // Could use different icons for incoming/outgoing if desired
  if (callType === 'video') return wasOutgoing ? '📹' : '📹';
  if (callType === 'screenshare') return '🖥️';
  return '📞'; // Default
});
...
async function loadMedia() { /* ... no change ... */ if (!isMediaFile.value || !props.message.fileHash) return; const url = await mediaCacheService.getUrl(props.message.fileHash); if (url) { displayUrl.value = url; } else { eventBus.on('file:ready', handleFileReady); } }
function handleFileReady({ fileHash }) { /* ... no change ... */ if (fileHash === props.message.fileHash) { log(`'file:ready' event received for message ${props.message.id}. Reloading media.`, 'DEBUG'); loadMedia(); eventBus.off('file:ready', handleFileReady); } }
function handleMediaClick() { /* ... no change ... */ if (displayUrl.value && (isImage.value || isVideo.value)) { uiStore.showMediaViewer({ type: isVideo.value ? 'video' : 'image', src: displayUrl.value, alt: props.message.fileName }); } else if (displayUrl.value) { const a = document.createElement('a'); a.href = displayUrl.value; a.download = props.message.fileName || 'download'; document.body.appendChild(a); a.click(); document.body.removeChild(a); } }
function toggleAudioPlay() { /* ... no change ... */ if (!displayUrl.value) return; if (!audioPlayer) { audioPlayer = new Audio(displayUrl.value); audioPlayer.onended = () => { isPlaying.value = false; }; audioPlayer.onpause = () => { isPlaying.value = false; }; audioPlayer.onplaying = () => { isPlaying.value = true; }; } isPlaying.value ? audioPlayer.pause() : audioPlayer.play().catch(e => log(`Error playing audio preview: ${e}`, 'ERROR')); }
function showContextMenu(event) { /* ... no change ... */ if (props.message.type === 'system' || props.message.isRetracted) return; const items = [{ label: '删除', action: () => uiStore.showConfirmationModal({ message: "确定要删除这条消息吗？此操作仅在您本地生效。", onConfirm: () => chatStore.deleteMessage(props.message.id) }), class: 'danger' }]; if (isMyMessage.value && Date.now() - new Date(props.message.timestamp).getTime() < AppSettings.ui.messageRetractionWindow) { items.push({ label: '撤回', action: () => chatStore.retractMessage(props.message.id) }); } uiStore.showContextMenu({ event, items, target: { type: 'message', id: props.message.id } }); }

watch(() => props.message.fileHash, loadMedia, { immediate: true });
onUnmounted(() => { /* ... no change ... */ eventBus.off('file:ready', handleFileReady); if (audioPlayer) { audioPlayer.pause(); audioPlayer.src = ''; audioPlayer = null; } });
</script>
...
/* --- NEW: Style for system message wrapper --- */
