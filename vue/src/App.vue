<template>
  <div id="app-root" :style="backgroundStyle">
    ...
    <!-- *** NEW: Confirmation and Media Viewer Modals *** -->
...
const themeHref = computed(() => {
  ...
  return themeConfig?.css ? `/${themeConfig.css}` : ''; // Vite serves from public root
});
...
onMounted(async () => {
  ...
  try {
    ...
    // 延迟一点时间移除加载动画，给Vue一点渲染时间，体验更佳
    setTimeout(() => uiStore.setAppLoading(false), 300);
  }
});

// --- 侦听器 ---
// 使用 watch 来动态修改 <body> 和 <html> 的 class
...
/* ... 样式保持不变 ... */
#app-root {
  width: 100vw; height: 100dvh; overflow: hidden; display: flex; align-items: center; justify-content: center;
  background-color: var(--color-background-page); color: var(--color-text-primary);
  font-family: var(--font-family-base); transition: background-image 0.5s ease-in-out;
  background-size: cover; background-position: center; background-image: var(--custom-background-image, none);
}
.app-container {
  display: grid; grid-template-columns: var(--sidebar-width) 1fr;
  width: 100%; height: 100%; max-width: var(--max-app-width); max-height: 95dvh;
  border-radius: var(--border-radius-lg); box-shadow: var(--shadow-lg); overflow: hidden; position: relative;
  transition: grid-template-columns 0.3s var(--transition-easing); background-color: var(--color-background-panel);
}
.app-container.details-panel-open { grid-template-columns: var(--sidebar-width) 1fr var(--details-panel-width); }
.sidebar-container, .main-view-container, .details-panel-container {
  display: flex; flex-direction: column; height: 100%; overflow: hidden;
}
.sidebar-container { background-color: var(--color-background-panel); }
.details-panel-container { background-color: var(--color-background-panel); border-left: 1px solid var(--color-border); }
.main-view-container { border-left: 1px solid var(--color-border); }
.loading-overlay {
  position: fixed; inset: 0; display: flex; flex-direction: column; align-items: center; justify-content: center;
  background-color: var(--color-background-page); z-index: 9999;
}
.loading-text { margin-top: 1rem; font-size: 1.2rem; color: var(--color-text-secondary); }

/* Responsive Breakpoints */
@media (max-width: 1024px) {
  .app-container.details-panel-open { grid-template-columns: var(--sidebar-width) 1fr; }
  .details-panel-container {
    position: absolute; top: 0; right: 0; width: var(--details-panel-width); height: 100%;
    transform: translateX(100%); transition: transform 0.3s var(--transition-easing); z-index: 100;
  }
  .app-container.details-panel-open .details-panel-container { transform: translateX(0); }
}
@media (max-width: 768px) {
  .app-container { grid-template-columns: 1fr; max-height: 100dvh; border-radius: 0; }
  .sidebar-container {
    width: 100%; position: absolute; top: 0; left: 0; height: 100%; z-index: 10;
    transform: translateX(0); transition: transform 0.3s var(--transition-easing);
  }
  .main-view-container { border-left: none; }
  .app-container.chat-active .sidebar-container { transform: translateX(-100%); }
  .details-panel-container { width: 85vw; max-width: 320px; }
}
...
html.is-transitioning::view-transition-new(root) {
  animation: reveal-in 0.5s ease-in-out; mix-blend-mode: normal;
}
