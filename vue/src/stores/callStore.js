...
export const useCallStore = defineStore('call', () => {
    // --- STATE (No new state needed, just logic changes) ---
    ...
    // --- NEW: Track call start time for accurate duration ---
    ...
    async function _playMusic(isRetry = false) { /* ... no change ... */ _initMusicPlayer(); if (musicPlayer && !isMusicPlaying) { try { await musicPlayer.play(); isMusicPlaying = true; if (boundEnableMusicPlay) { document.body.removeEventListener('click', boundEnableMusicPlay); boundEnableMusicPlay = null; } } catch (error) { log(`播放呼叫音乐失败: ${error.name} - ${error.message}`, 'WARN'); isMusicPlaying = false; if (error.name === 'NotAllowedError' && !isRetry) { eventBus.emit('showNotification', { message: '浏览器阻止了铃声自动播放。请点击页面任意位置以启用声音。', type: 'warning' }); boundEnableMusicPlay = () => _playMusic(true); document.body.addEventListener('click', boundEnableMusicPlay, { once: true }); } } } }
    function _stopMusic() { /* ... no change ... */ if (musicPlayer && isMusicPlaying) { musicPlayer.pause(); musicPlayer.currentTime = 0; isMusicPlaying = false; } if (boundEnableMusicPlay) { document.body.removeEventListener('click', boundEnableMusicPlay); boundEnableMusicPlay = null; } }

    function _startCallTimer() {
        if (callTimer) clearInterval(callTimer);
        callStartTime = Date.now(); // --- NEW: Record start time ---
        callDuration.value = 0;
        callTimer = setInterval(() => {
            callDuration.value = Math.round((Date.now() - callStartTime) / 1000);
        }, 1000);
    }
    ...
    function _stopCallTimer() {
        if (callTimer) clearInterval(callTimer);
        callTimer = null;
        // Do not reset duration here, hangUp needs it
    }
    ...
    function _resetState(keepPeerId = false) {
        ...
        if (!keepPeerId) currentPeerId.value = null;
        ...
        isFullScreenCallViewVisible.value = false;
        _stopMusic();
        _stopCallTimer();
        if (callRequestTimeout) clearTimeout(callRequestTimeout);
        callRequestTimeout = null;
        callStartTime = null; // --- NEW ---
    }

    async function _getMediaStream(options = { video: true, audio: true, screen: false }) { /* ... no change ... */ try { if (options.screen) { const screenStream = await navigator.mediaDevices.getDisplayMedia({ video: { cursor: "always" }, audio: true }); try { const micStream = await navigator.mediaDevices.getUserMedia({ audio: AppSettings.media.audioConstraints, video: false }); micStream.getAudioTracks().forEach(track => screenStream.addTrack(track)); } catch (micError) { log(`Could not get microphone for screen share: ${micError.message}`, 'WARN'); eventBus.emit('showNotification', { message: '无法获取麦克风，将继续共享但不包含您的声音。', type: 'warning' }); } screenStream.getVideoTracks()[0].onended = () => hangUp(); return screenStream; } return await navigator.mediaDevices.getUserMedia({ video: options.video, audio: options.audio ? AppSettings.media.audioConstraints : false }); } catch (error) { log(`Failed to get media stream: ${error.message}`, 'ERROR'); eventBus.emit('showNotification', { message: `无法访问摄像头或麦克风: ${error.message}`, type: 'error' }); return null; } }
    function _initiateMediaSession(peerId, options = { isScreenShare: false, audioOnly: false }) { /* ... no change ... */ if (isCallActive.value || isCallPending.value) { eventBus.emit('showNotification', { message: '已在通话中', type: 'warning' }); return; } const userStore = useUserStore(); currentPeerId.value = peerId; isCaller.value = true; isCallPending.value = true; isScreenSharing.value = options.isScreenShare; isVideoEnabled.value = !options.isScreenShare && !options.audioOnly; const callType = options.isScreenShare ? 'screenshare-request' : 'call-request'; webrtcService.sendMessage(peerId, { type: callType, from: userStore.userId, audioOnly: options.audioOnly }); useUiStore().showModal('calling'); _playMusic(); callRequestTimeout = setTimeout(() => { if (isCallPending.value) { eventBus.emit('showNotification', { message: '对方无应答', type: 'info' }); webrtcService.sendMessage(peerId, { type: 'call-cancel', from: userStore.userId }); _resetState(); useUiStore().hideModal(); } }, AppSettings.timeouts.callRequest); }
    ...
    async function acceptCall() {
        ...
        const { peerId, audioOnly, isScreenShare } = incomingCallInfo.value; // Get isScreenShare
        _stopMusic();
        uiStore.hideModal();
        ...
        isCallPending.value = false;
        isScreenSharing.value = isScreenShare; // Set screen sharing state
        ...
        // --- NEW: Record Call Start ---
        _startCallTimer();
        useChatStore().addCallLogMessage(peerId, {
            type: 'start',
            callerId: peerId, // The person who initiated the call
            callType: isScreenShare ? 'screenshare' : (audioOnly ? 'audio' : 'video')
        });
    }

    function rejectCall(isInternal = false) { /* ... no change ... */ let peerIdToNotify; if (isCaller.value && isCallPending.value) { peerIdToNotify = currentPeerId.value; if (!isInternal) webrtcService.sendMessage(peerIdToNotify, { type: 'call-cancel', from: useUserStore().userId }); } else if (incomingCallInfo.value) { peerIdToNotify = incomingCallInfo.value.peerId; if (!isInternal) webrtcService.sendMessage(peerIdToNotify, { type: 'call-rejected', from: useUserStore().userId }); } _resetState(); useUiStore().hideModal(); }
    ...
    function hangUp(notifyPeer = true) {
        ...
        _stopCallTimer(); // Stop timer before state reset
        ...
        if (isCallActive.value) {
            useChatStore().addCallLogMessage(peerId, {
                ...
                callType: isScreenSharing.value ? 'screenshare' : (!isVideoEnabled.value ? 'audio' : 'video')
            });
        }
        ...
        _resetState(true); // Keep peerId for potential immediate follow-up
    }

    function toggleAudio() { /* ... no change ... */ if (!localStream.value) return; localStream.value.getAudioTracks().forEach(track => { track.enabled = !track.enabled; isAudioMuted.value = !track.enabled; }); }
    function toggleVideo() { /* ... no change ... */ if (!localStream.value || isScreenSharing.value) return; localStream.value.getVideoTracks().forEach(track => { track.enabled = !track.enabled; isVideoEnabled.value = track.enabled; }); }
    ...
    eventBus.on('webrtc:stream', ({ peerId, stream }) => { /* ... no change ... */ if (currentPeerId.value === peerId) { if (stream instanceof MediaStream) remoteStream.value = stream; else log(`Received invalid stream from peer ${peerId}.`, 'WARN'); } });
    ...
    return {
        ...
        isVideoEnabled, isScreenSharing, incomingCallInfo, isFullScreenCallViewVisible, callDurationFormatted, peerContact,
        startVideoCall, startAudioCall, startScreenShare,
        acceptCall, rejectCall, hangUp, toggleAudio, toggleVideo,
        minimizeCallView, maximizeCallView,
    };
});