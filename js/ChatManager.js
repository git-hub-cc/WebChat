 ...
 *              REFACTORED: (第2阶段) 不再直接调用 SidebarUIManager，而是通过 Store 更新标签页状态。
 * @module ChatManager
 * @exports {object} ChatManager - 对外暴露的单例对象，包含所有聊天管理功能。
 * @dependencies DBManager, UserManager, GroupManager, ConnectionManager, MessageManager, DetailsPanelUIManager, ChatAreaUIManager, SidebarUIManager, NotificationUIManager, Utils, ModalUIManager, Store
...
const ChatManager = {
    ...
    renderChatList: function(filter = 'all') {
        ...
        if (typeof Store !== 'undefined') {
            Store.dispatch('SET_ACTIVE_TAB', { tab: filter });
        }
    ...
    /**
     * REFACTORED: 打开指定的聊天会话。现在通过 Store 触发状态更新。
     * @param {string} chatId - 要打开的聊天 ID。
     * @param {string} type - 聊天类型 ('contact' 或 'group')。
     */
    openChat: function(chatId, type) {
        // 如果点击的是当前已打开的聊天，则不执行任何操作（或仅在移动端切换视图）
        // Store 的 reducer 会处理这种情况，确保状态不会冗余更新。
        // if (this.currentChatId === chatId) {
        //     // 移动端切换视图的逻辑现在由 LayoutUIManager 响应 Store 变化来处理
        //     return;
        // }
        ...
        // --- 核心修改 ---
        // 分发一个 action 来通知应用状态已变更，而不是直接调用 UI 管理器。
        // UI 管理器们 (ChatAreaUIManager, LayoutUIManager, DetailsPanelUIManager)
        // 将会订阅 Store，并根据这个新的 currentChatId 来更新自己。
        Store.dispatch('OPEN_CHAT', { chatId: chatId });

        // 渲染列表以更新高亮项（这可以优化为让 SidebarUIManager 也订阅Store）
        this.renderChatList(this.currentFilter);

        // 清空输入框和聚焦的逻辑可以保留，因为它不直接影响其他模块的状态
    ...
    deleteChat: function(chatId, type) {
        // ... REFACTORED (Phase 2): 移除直接 UI 调用，使用 Store ...
