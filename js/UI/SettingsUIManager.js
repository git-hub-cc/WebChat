...
// BUG FIX: 将新用户的默认提供商从 'siliconflow' 改为 'ppmc'，以确保 UI 与后端逻辑一致。
    ...
    // --- BUGFIX: Logic updated to ensure model consistency ---
    _handleLlmProviderChange: function(providerKey) {
        ...
        // 3. Update the model input/selector UI with the determined effective model
        this._updateModelInput(providerConfig, effectiveModelKey);
    },
    // --- BUGFIX: _updateModelInput now accepts the effective model key ---
    _updateModelInput: function(providerConfig, effectiveModelKey) {
        this.apiModelInputContainer.innerHTML = ''; // Clear previous input/selector

        // If no preset model list (e.g., "Custom" provider), create a text input
        if (!providerConfig.models || providerConfig.models.length === 0) {
            ...
            // Otherwise, create the custom div-based dropdown
            ...
            const storedModelKey = effectiveModelKey; // Use the passed effective model key
